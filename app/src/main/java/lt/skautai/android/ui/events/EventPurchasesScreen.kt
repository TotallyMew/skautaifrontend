            ...
            when (state) {
                ...
                is EventPurchasesUiState.Success -> {
                    LazyColumn(
                        ...
                        item {
                            PurchasesCard(
                                eventNotes = state.event.notes,
                                purchases = state.purchases,
                                canManage = canInventory,
                                isWorking = state.isWorking,
                                onCompletePurchase = { purchaseId, totalAmount ->
                                    viewModel.completePurchase(eventId, purchaseId, totalAmount)
                                },
                                onAddPurchaseToInventory = { purchaseId ->
                                    viewModel.addPurchaseToInventory(eventId, purchaseId)
                                },
                                onAttachInvoice = { purchaseId, uri ->
                                    viewModel.attachInvoice(eventId, purchaseId, uri)
                                },
                                onDownloadInvoice = { purchaseId ->
                                    viewModel.downloadInvoice(eventId, purchaseId)
                                }
                            )
                        }
                    }
